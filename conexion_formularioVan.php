...
$conexion = mysqli_connect('localhost', 'root', '04272931', 'practica_ipi')
  ...
  function insertar ($conexion){
    ...
    $idEmpresas= "select idEmpresas from empresas where Nombre_Empresa = '$nombre_empresa' ";
    $validarId = mysqli_query($mysqli, $idEmpresas); 
    
    if(mysqli_num_rows($validarId) > 0 ) {
        $resultado = $mysqli->query($idEmpresas);
